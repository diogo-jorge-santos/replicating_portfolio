    ...
    def replicating_error(self):
        ...
        for j in range(0, self.n_paths):
            ...
            q[0] = self.pricing(path[0], 0)-d[0]*path[0] - \
                np.abs(d[0])*self.transaction_cost*path[0]
            ...
            path[self.n_steps] = path[self.n_steps-1]*np.exp((self.actual_mean-0.5*self.actual_vol**2)*(
                self.delta_t)+self.actual_vol*np.sqrt(self.delta_t)*np.random.normal())
...
class Short_european_call_bs(Option_fixed_time):
    ...
    def pricing(self, s, t):
        ...
        c0 = s*sp.norm.cdf(d1)-self.strike_price * \
            np.exp(-self.interest_rate*(self.expire_date-t))*sp.norm.cdf(d2)
...
class Short_european_call_leland(Option_fixed_time):
    def __init__(self, price_0, actual_mean, actual_vol, interest_rate, expire_date, strike_price, transaction_cost, n_steps, n_paths):
        Option_fixed_time.__init__(self, price_0, actual_mean, actual_vol, interest_rate,
                                   expire_date, strike_price, transaction_cost, n_steps, n_paths)
        self.modified_vol = np.sqrt((self.actual_vol**2) * (
            1+(self.transaction_cost/self.actual_vol)*np.sqrt(8/(self.delta_t*np.pi))))
...
if __name__ == "__main__":
    ...
    time_0 = time.time()
    option = Short_european_call_bs(price_0=100, actual_mean=0.05, actual_vol=0.25, interest_rate=0.05,
                                    expire_date=1, strike_price=100, transaction_cost=0.01, n_steps=100, n_paths=200)
    result = option.replicating_error()

    print(np.average(result))
    print(np.std(result))

    option1 = Short_european_call_leland(price_0=100, actual_mean=0.05, actual_vol=0.25, interest_rate=0.05,
                                         expire_date=1, strike_price=100, transaction_cost=0.01, n_steps=100, n_paths=200)
