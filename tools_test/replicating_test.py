...
def replicating_error_black_sholes(price_0, actual_mean, actual_vol, interest_rate, expire_date, strike_price, transaction_cost, n_steps, n_paths):
    ...
    for j in prange(0, n_paths):
        ...
        d[0] = delta(path[0], expire_date, 0, actual_vol,
                     interest_rate, strike_price)
        # rf bank account at t=0- revenue of selling the option
        q[0] = bs_pricing(path[0], expire_date, actual_vol, interest_rate,
                          strike_price)-d[0]*path[0]-np.abs(d[0])*path[0]*transaction_cost

        for i in range(1, n_steps):
            # genarate next price using the gbm formula
            path[i] = path[i-1]*np.exp((actual_mean-0.5*actual_vol**2)
                                       * delta_t + actual_vol*np.sqrt(delta_t)*np.random.normal())
            # quantity of stocks- equal to the delta of the option
            d[i] = delta(path[i], expire_date, i*delta_t,
                         actual_vol, interest_rate, strike_price)
            # rf bank account- (last value+interest) + cost/revenue of buying stocks + TC of buying stocks
            q[i] = q[i-1]*compound - (d[i]-d[i-1])*path[i] - \
                np.abs(d[i]-d[i-1])*path[i]*transaction_cost

        # genarate final price of the stock
        path[n_steps] = path[n_steps-1] * \
            np.exp((actual_mean-0.5*actual_vol**2)*delta_t +
                   actual_vol*np.sqrt(delta_t)*np.random.normal())
        ...
        error[j] = q[n_steps-1]*compound+d[n_steps-1] * \
            path[n_steps] - exercise(path[n_steps], strike_price)
...
def replicating_error_leland(price_0, actual_mean, actual_vol, interest_rate, expire_date, strike_price, transaction_cost, n_steps, n_paths):
    ...
    modified_vol = actual_vol * \
        np.sqrt(1 + (transaction_cost/actual_vol)
                * np.sqrt(8 / (delta_t*np.pi)))
    ...
    for j in prange(0, n_paths):
        ...
        path[0] = price_0
        # quantity of stocks- equal to the delta of the option
        d[0] = delta(path[0], expire_date, 0, modified_vol,
                     interest_rate, strike_price)
        # rf bank account at t=0- revenue of selling the option
        q[0] = bs_pricing(path[0], expire_date, actual_vol, interest_rate,
                          strike_price)-d[0]*path[0]-np.abs(d[0])*path[0]*transaction_cost
        ...
        for i in range(1, n_steps):
            ...
            # quantity of stocks- equal to the delta of the option
            d[i] = delta(path[i], expire_date, i*delta_t,
                         modified_vol, interest_rate, strike_price)
            # rf bank account- (last value+interest) + cost/revenue of buying stocks + TC of buying stocks
            q[i] = q[i-1]*compound - (d[i]-d[i-1])*path[i] - \
                np.abs(d[i]-d[i-1])*path[i]*transaction_cost
...
def aux():
    ...
    for i in prange(1, 41):
        ...
        option = replicating_error_black_sholes(price_0=100, actual_mean=0.05, actual_vol=0.25, interest_rate=0.05,
                                                expire_date=1, strike_price=100, transaction_cost=0.01, n_steps=delta_t, n_paths=200000)
        option1 = replicating_error_leland(price_0=100, actual_mean=0.05, actual_vol=0.25, interest_rate=0.05,
                                           expire_date=1, strike_price=100, transaction_cost=0.01, n_steps=delta_t, n_paths=200000)
...
if __name__ == "__main__":
    ...
    time0 = time.time()
    option = replicating_error_black_sholes(price_0=100, actual_mean=0.05, actual_vol=0.25, interest_rate=0.05,
                                            expire_date=1, strike_price=100, transaction_cost=0.01, n_steps=100, n_paths=200000)
