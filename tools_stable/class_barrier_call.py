...
class Up_and_out_call(Option_barrier_in):
    ...
    def pricing(self, s, t, barrier_in):
        ...
        price_1 = self.bs_call(s, t, self.strike_price) - self.bs_call(s, t, self.barrier_price) -\
            (self.barrier_price - self.strike_price) * np.exp(-self.interest_rate *
                                                              (self.expire_date - t)) * normcdf(self.d2(s, t, self.barrier_price))

        price_2 = ((self.barrier_price / s)**(2 * mu / (self.actual_vol**2))) * (self.bs_call((self.barrier_price**2) / s, t, self.strike_price) -
                                                                                 self.bs_call((self.barrier_price**2) / s, t, self.barrier_price) - (self.barrier_price - self.strike_price) * np.exp(-self.interest_rate * (self.expire_date - t)) * normcdf(self.d2(self.barrier_price, t, s)))
    ...
    def barrier(self, s):
        '''
        Function to check if spot is over the barrier
        s: spot price
        '''
...
def risk_return_barrier_call(
    ...
    list = np.empty(200)
    list_risk = np.empty(200)

    list_cost = np.empty(200)
    list_cost_risk = np.empty(200)

    list_slipage = np.empty(200)
    list_slipage_risk = np.empty(200)
...
def gamma_test_barrier_call(
    ...
    list_abs = np.empty(200)
    list_square = np.empty(200)
