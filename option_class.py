...
class Short_european_call_black_sholes(Option_fixed_time):
    ...
    def pricing(self,s,t):
        ...
        c0=s*normcdf(d1)-self.strike_price*np.exp(-self.interest_rate*(self.expire_date-t))*normcdf(d2)
...
class Short_european_call_leland(Option_fixed_time):
    ...
    def __init__(self,price_0,actual_mean,actual_vol,interest_rate,expire_date,strike_price,transaction_cost,n_steps,n_paths):
        ...
        self.__init__A(price_0,actual_mean,actual_vol,interest_rate,expire_date,strike_price,transaction_cost,n_steps,n_paths)
        #its needed to add the modified volatility to the class objects
        self.modified_vol=  self.actual_vol *np.sqrt(  1    +    (self.transaction_cost/self.actual_vol)    *  np.sqrt(   8    /    (self.delta_t*np.pi))   )
...
def frontier_parallel(price_0,actual_mean,actual_vol,interest_rate,expire_date,strike_price,transaction_cost,n_paths):
    ...
    for i in prange(1,41):
        ...
        option=Short_european_call_black_sholes(price_0,actual_mean,actual_vol,interest_rate,expire_date,strike_price,transaction_cost,delta_t,n_paths)
        option1=Short_european_call_leland(price_0,actual_mean,actual_vol,interest_rate,expire_date,strike_price,transaction_cost,delta_t,n_paths)
